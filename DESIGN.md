...
> 设计基调:**明亮精密控制台 + 炭墨文字 + 翡翠主色**。目标观感:专业、快速、可信,像一台无需学习成本的文件处理工作台。

## 1. 视觉主题与氛围

- **气质**:明亮、精密、克制。科技感来自清晰的状态、紧凑的信息层级、线性图标与发丝线,不依赖霓虹或深色大屏。
- **画布**:全局使用暖白 `#fbfbfa`,内容承载在纯白 `#ffffff` 表面上,以暖色发丝线组织结构。
- **主色**:翡翠绿(emerald),代表"本地处理、安全、即时"。云端处理功能统一用天蓝(sky)标注。
- **明暗**:仅浅色主题,无暗色模式。
- **圆角语言**:精密小圆角——控件 `rounded-xl`,列表图标与标签 `rounded-lg`,工作区容器 `rounded-2xl`;胶囊仅用于状态点。禁止无边界的卡片墙。
...
- **卡片/列表**:优先白底 + `border-slate-200`;目录页使用整行列表与分隔线,不使用大面积悬浮卡片墙。
- **工作区容器**(每个工具页的操作区):`rounded-2xl border border-slate-200 bg-white shadow-card`。
...
- 页面容器:首页桌面采用 `15.5rem` 分类侧栏 + 主工作区,整体 `max-w-[96rem]`;工具详情 `max-w-6xl`。
- 工作区内部:左右分栏 `grid lg:grid-cols-[1fr_0.55fr~0.85fr] gap-6`,窄屏自动堆叠。
- 区块间距:首页主要区块 `py-10 sm:py-14`,依靠边框与字号层级而非超大留白分组。
- 工具页固定结构:返回链接 → 英文状态 eyebrow(`tracking-[0.18em]` 小字)→ H1 → 长描述 → 工作区 → 发丝线「如何使用」三步流程。
...
边框优先于阴影表达结构;阴影只用于搜索框与核心工作区的轻微抬升。

## 7. 设计规范与禁忌

✅ 应该:新组件复用上述类名组合;新工具接入 `tools.ts` 自动获得规范;反馈一律走底部消息横幅;空态/禁用态必须给出下一步指引文案。

❌ 禁止:引入新的品牌色相(紫/粉仅限分类色板);使用冷灰(blue-gray/slate 默认值);霓虹、渐变、玻璃拟态、伪造系统指标;没有 `aria-live` 的静默报错;把「云端处理」标成绿色;硬编码字体名覆盖字族栈。

## 8. 响应式行为

- 断点:移动优先,主要断点 `sm(640) / lg(1024)`。
- 移动端:桌面分类侧栏变为横向可滚动分类条;常用工具两列;目录行隐藏次要描述并保留名称、处理方式与触控目标。
- 工具页分栏纵向堆叠;工作区内三列选项组 `sm:grid-cols-3` 堆叠;触控目标最小 `44px`。
